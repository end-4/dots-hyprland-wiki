...
Consider someone who forks the repo and modifies something in the PKGBUILDs. It just works.
However if the PKGBUILDs are on AUR then this user will have to download PKGBUILDs and upload them to AUR with a new name... What if many users are doing this altogether? That's just unacceptable.

**2.** These local PKGBUILDs are actually only for illogical-impulse and does not really add new "packages". It's unsuitable to be uploaded to AUR as it's a public place for holding packages for everyone.
...
None of them are uploaded officially. We have no control over them.
