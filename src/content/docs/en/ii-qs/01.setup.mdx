...
It clones the repo to `~/.cache/dots-hyprland` by default and run `./install setup` for you.
...
- If you think it's just fine, then just run `./install -f --skip-alldeps --skip-allfiles` which will run this script.
  - Do not run `sdata/dist-arch/install-setups.sh` directly because it's meant to be sourced by `./setup install`.
...
- `cd` to the repo directory and run `git pull` to fetch the latest changes.
