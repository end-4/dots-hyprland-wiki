...
:::note[Distro support]
- Automated: see the section [Automated Installation/Distro Support](#distro-support).
- Manually: see the section [Manual Installation](#manual-installation).
- Forks: see the section [Forks](#forks).
:::
...
- **Officially supported:**
  - Arch Linux
    - [clsty](https://github.com/clsty)(the main developer of install scripts) uses and recommends Arch Linux: "vanilla Arch Linux just works. EndeavourOS is also OK because it's using same package repo as Arch Linux."
  - EndeavourOS
...
- **Partially supported:**
  - Other Arch(-based) distros, such as CachyOS, Manjaro, etc.
    - These distros use different package repos different as Arch Linux, so it's possible that things do not work as expected.
...
```
The above clones the repo to `~/.cache/dots-hyprland`, and then run `./install.sh` for you.
...
Note that some of them are for the AGS version though.
...
**This method is strongly discouraged cuz it can cost you lots of time.**
