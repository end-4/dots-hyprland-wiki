...
You only need to run this one line:
```bash
bash <(curl -s "https://end-4.github.io/dots-hyprland-wiki/setup.sh")
```
That's it!
- Just follow the prompts until finish.
- Behind the scene, it clones the repo to `~/.cache/dots-hyprland` and go there, then run `./install.sh` for you.
...
To clone to somewhere else, specify a path directly as an argument, e.g.
```bash
