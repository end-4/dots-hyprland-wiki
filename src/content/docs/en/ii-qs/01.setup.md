...
- If you think it's just fine, then just run `./install -f --skip-alldeps --skip-allfiles` which will run this script.
  - Do not run `sdist/arch/install-setups.sh` directly because it's meant to be sourced by `install.sh`.
- Or, run the lines in this script manually with your tweaks.

### Copying config files
- Copy `dots/.config`, `dots/.local` to your home folder (review before doing this or you might have your own config files undesirably overwritten)
- Or, read `sdata/install-files.sh`.
  - If you think it's just fine, then just run `./install.sh --skip-alldeps --skip-allsetups` which will run this script.
  - Do not run `sdata/install-files.sh` directly because it's meant to be sourced by `install.sh`.
...
the `install.sh --exp-uninstall` is capable of a suitable uninstallation for Arch/AUR packages, 
...
- You can try using `install.sh --exp-uninstall`, but it's not perfect and not actively maintained.
