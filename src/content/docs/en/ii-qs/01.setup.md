...
- **Officially supported:**
  - Arch Linux
    - [clsty](https://github.com/clsty) (the main developer of install scripts) uses and recommends Arch Linux: "The vanilla Arch Linux just works. EndeavourOS is also OK because it's using almost the same package repos (except [this one](https://github.com/endeavouros-team/repo)) as Arch Linux."
  - EndeavourOS
    - [end-4](https://github.com/end-4) (the author of illogical-impulse) uses and recommends EndeavourOS: "You don't have to, but it makes things much easier compared to normal Arch."
- **Partially supported:**
...
`install.sh` is now designed to select distro automatically,
...
- Behind the scene, it clones the repo to `~/.cache/dots-hyprland` and `cd` there, then run `./install.sh`.

:::note[Changed domain]
You might've noticed that the above link (and the wiki's address) has changed. This is to give you a shorter link to type in case you're doing this from a tty, and content is still served by GitHub Pages. If you add the `-L` flag to the above `curl` command and use the link `https://end-4.github.io/dots-hyprland-wiki/setup` it would also work
:::

:::tip[Using install.sh]
...
:::tip[Alternative steps]
If you want to type a shorter link, then
```bash
...
- `dist-fallback/install-deps.sh` also contains alternative installing functions for some dependencies, which are not distro-specific but out of control from the system package manager. Better avoid using them if possible.
- If you've successfully finished installation on some non-Arch(-based) Linux distro, you may create an `install-<DISTRO_ID>/install-deps.sh` and submit a PR.
