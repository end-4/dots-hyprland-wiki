...
`install.sh` is now designed also for non-Arch(-based) distros via two methods:
1. When `--via-nix` is passed to `install.sh`, `scriptdata/install-deps-nix.sh` (still WIP) will be used. It's not for NixOS, but for nearly every distro.
  - **We need help on `scriptdata/install-deps-nix.sh`.** See [issue #1061](https://github.com/end-4/dots-hyprland/issues/1061).
2. When `--via-nix` is not passed, `./install.sh` detects the distro ID and try to use `scriptdata/install-deps-<DISTRO_ID>.sh` for dependency installation if it exists.
  - **We need more scripts `scriptdata/install-deps-<DISTRO_ID>.sh`** to support specific distro using their system package manager.
...
P.S. similar to `scriptdata/install-setup-*.sh`, but generally most distros can just use the `scriptdata/install-setup-fallback.sh` for this part, so it's not a big deal.
:::
