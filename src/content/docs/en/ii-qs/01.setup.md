...
:::note[Supported Distros]
- **Supported:**
  - **Arch Linux** and **EndeavourOS** are supported and used by the developers. The former is more minimal, while the latter offers a simplified installation.
  - **CachyOS** is using different repositories than official Arch, but it's designed to be compatible with AUR so ideally it should also work.
  - Other Arch-based distros using different repositories than official Arch are only partially supported, especially **Manjaro** which has relatively poor compatibility with AUR.
