...
Note: The `OS_GROUP_ID` is similar to `OS_DISTRO_ID` but correspond to a group of distros.
- e.g. Arch Linux, EndeavourOS, CachyOS and other Arch(-based) distros corresponds to `OS_GROUP_ID=arch`.
- e.g. Opensuse Leap and Opensuse Tumbleweed should corresponds to `OS_GROUP_ID=suse`.
