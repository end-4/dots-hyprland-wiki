...
For now, you just need to edit `~/.config/ags/user_options.js`.
...
- For the statusbar, edit `~/.config/ags/modules/bar/system.js`. Change `"%H:%M"` according to the format described [here](https://docs.gtk.org/glib/method.DateTime.format.html).

- For lock screen: edit `~/.config/hypr/hyprlock.conf`. Change the value of the `text` **which corresponds to the lock**, according to [Hyprland Wiki](https://wiki.hyprland.org/Hypr-Ecosystem/hyprlock/).
