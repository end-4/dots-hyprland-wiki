...
# Wallpaper & colors
- To pick a wallpaper, press `Ctrl`+`Super`+`T` and select an image. Widgets, Gtk apps (after restarting them), and Qt apps (see [how to set it up properly](https://ii.clsty.link/en/i-i/01setup/#not-so-optional-stuff)) will be themed with Material colors.
