...
- If you have a wallpaper directory and would like to change wallpappers quickly, edit the file `~/.config/ags/scripts/color_generation/randomwall.sh`.
    - You must install the `fd` command on your system. Edit the path with your wallpaper directory.
    - If set-up properly, executing the script should randomly select a wallpaper to display.
    - Consider adding a keybind to `custom/keybinds.conf` to change wallpapers on the fly.

- To automate changing wallpapers, you can write a script which waits for an amount of time before randomly selecting a new wallpaper.
- Add the script to `custom/execs.conf` to start the wallpaper automation process on boot.
