...
To configure Hyprland, edit `~/.config/hypr/hyprland.conf` (the main config file) and files in the `~/.config/hypr/hyprland` directory (files in which are sourced by `hyprland.conf`).
...
The city for weather is detected by `curl ipinfo.io` by default, or explicitly set as the environment variable `$AGS_WEATHER_CITY`.
