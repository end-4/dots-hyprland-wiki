...
# Focus mode <sup>experimental</sup>
...
But if you find something wrong with Hyprland after updating using `install.sh`, you need to re-check `~/.config/hypr/hyprland.conf` to see if the commented-out default config file contains the configuration which should have been applied.
:::
...
Configuring AGS requires a relatively high level of technical expertise compared to other "frameworks" such as EWW (refer to [AGS Docs](https://aylur.github.io/ags-docs) and also Dev Notes here if you're still interested).
However, we have managed to reduce the complexity of some common configurations.

For now, you just need to edit `~/.config/ags/user_options.js`, which will not be overwritten when running `install.sh`.
This file contains many options, such as:
- The city for weather widget (see details below)
- Scale of overview widget
...
In `~/.config/ags/user_options.js`, edit the "time" section.

Reference also [here](https://docs.gtk.org/glib/method.DateTime.format.html).
...
Instead, it executes the script repeatedly with every interval of 5000ms (default), and displays the output.
...
:::
:::caution
To make the script executable, you need to set execution permission:
```bash
...
- Create and edit `~/.cache/ags/user/scripts/custom-module-interval.txt` as the number of miliseconds for polling interval (default 5000).
- Create and edit `~/.cache/ags/user/scripts/custom-module-leftclick.sh` and it will be executed when left-clicking on the widget.
- Create and edit `~/.cache/ags/user/scripts/custom-module-rightclick.sh` and it will be executed when right-clicking on the widget.
...
## Change time format for lock screen
**NOTE**: You may need to run `man date` for time format.

Edit `~/.config/hypr/hyprlock.conf`. Change the value of the `text` **which corresponds to the lock**, according to [Hyprland Wiki](https://wiki.hyprland.org/Hypr-Ecosystem/hyprlock/).
