...
### How to auto-launch Hyprland when entering `tty1`?
For ZSH or BASH, add this line to the end of your `~/.zshrc` or `~/.bashrc`:
```bash
source ~/.config/zshrc.d/auto-Hypr.sh
```

For FISH, add this line to the end of your `~/.config/fish/config.fish`:
```fish
...
You may see `tty` as the "basis" of a Linux system.
...
After logging in, you're able to launch a graphical environment, e.g. `Hyprland`.
...
How do the DM know which graphical environment is available?
Normally, it searches the path `/usr/share/xsessions` for X11 ones, and `/usr/share/wayland-sessions` for wayland ones. These directories contain the desktop files which contain the information of a graphical environment.
