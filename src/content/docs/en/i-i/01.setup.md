...
How does the DM know which graphical environment is available?
- Normally, it searches the path `/usr/share/xsessions` for X11 ones, and `/usr/share/wayland-sessions` for wayland ones.
- These directories contain the desktop files which contain the information of a graphical environment.
