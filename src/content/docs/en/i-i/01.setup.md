...
Just run the following command. The repo containing will be cloned to `~/.cache/dots-hyprland` by default.
```bash
bash <(curl -s "https://end-4.github.io/dots-hyprland-wiki/setup.sh")
```
You can specify a path for the repo by specifying it as the first argument.
```bash
