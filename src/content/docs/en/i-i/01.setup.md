...
- To explicitly update AGS program (not its config), run `./update-ags.sh`.
  - `./install.sh` will not update AGS program **automatically** for you to prevent redundant rebuild of AGS when you run it under `-f` flag, or when your tell it to execute automatically during running. Otherwise `./install.sh` will still ask you whether to install AGS for updating, just answer yes so it will update AGS program for you.
  - You can also run `./update-ags.sh` to update AGS program.
