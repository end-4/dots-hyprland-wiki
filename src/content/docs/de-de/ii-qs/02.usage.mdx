...
Diese Seite dokumentiert einige nicht offensichtliche Details. Nicht alles steht hier, also erkunde...

# Allgemein

Drücke `Super`+`/` für eine Liste der Tastenkürzel
...
- `Super`+`Shift`+`T`: Text recognition
...
:::note
Workspace groups are not natively supported by Hyprland or any status bar/widget system. This config achieves this simply by tinkering with Hyprland dispatchers and the shell config.
:::
