...
- Hyprland wiki 建议使用 uswm 管理的会话启动 Hyprland，但我们不这样做。使用这个不会破坏 dotfiles，但你可能会从其他桌面环境获得自动启动的垃圾（例如重复的身份验证的对话框）
...
对于 ZSH 或 BASH，将此行添加到你的 `~/.zshrc` 或 `~/.bashrc` 的**底部**：
```bash
source ~/.config/zshrc.d/auto-Hypr.sh
```

对于 FISH，将此行添加到你的 `~/.config/fish/config.fish` 的**底部**：
```fish
...
- 它为你提供图形界面来登录和选择图形环境（例如 Hyprland）。
  - DM 如何知道哪些图形环境可用？
    - 通常，它搜索路径 `/usr/share/xsessions` 查找 X11 的，搜索 `/usr/share/wayland-sessions` 查找 Wayland 的。
    - 这些目录下的桌面文件包含图形环境的信息。
...
- 运行 `git pull` 获取最新更改。
- 再次运行 `./install.sh`
  - 跳过你不想要的步骤（特别是 `rsync ...` 的步骤，因为它会**覆盖其目标路径下的文件**）。
通常你可能想要涉及 `.config/quickshell` 的 `rsync` 步骤。

运行 `./install.sh -h` 查看更多用法。
## 如果你手动安装
- `cd` 到仓库目录
- 运行 `git pull` 获取最新更改。
- 获取你想要的文件。通常你可能想要获取 `.config/quickshell` 文件夹。
